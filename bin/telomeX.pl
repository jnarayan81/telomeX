...
telomeX.pl - telomeX script by Jitendra Narayan

=head1 SYNOPSIS

telomeX.pl --detect/-d

	telomeX.pl --detect/-d --conf/-c <configuration file>

telomeX.pl --merge/-m

	telomeX.pl --merge/-m --conf/-c <configuration file>

telomeX.pl --plot/-p

	telomeX.pl --plot/-p --conf/-c <configuration file>

telomeX.pl --annot/-a

	telomeX.pl --annot/-a --conf/-c <configuration file>
=cut
...
use lib "$FindBin::Bin/.";
require 'telomeX_module.pm';

#Basics mandatory telomeX variables
my (
$outfile, 	# Name for telomeX's main configuration file
$conf,
$detect,
$merge,
$plot,
...
# Default settings here for telomeX
my $current_version = "0.1";	#telomeX version
my %opts; my $nam;
...
Citation - telomeX: an open and parallel tool for automated G-quadruplexes analysis 
...
my $telomeX_path = dirname(rel2abs($0)); #print " Path of the dir\n$telomeX_path --------\n";

# Parameters_ref - stores all user-defined parameters, such as file locations and program parameters
my $param_ref = read_config_files(\$project_config_file, \$telomeX_path);  # stores the configuration in a hash reference
...
if (!-e "$param_ref->{out_dir}/results") { mkdir ("$param_ref->{out_dir}/results") || die ("Couldn't create the directory with the results of telomeX's analysis.\nDetails: $!\n"); }
if (!-e "$param_ref->{out_dir}/intermediate_files") { mkdir ("$param_ref->{out_dir}/intermediate_files/") || die ("Couldn't create the directory with the steps of telomeX's analysis.\nDetails: $!\n"); }
if (!-e "$param_ref->{out_dir}/intermediate_files/telomere") { mkdir ("$param_ref->{out_dir}/intermediate_files/telomere") || die ("Couldn't create the directory with the steps of telomeX's analysis.\nDetails: $!\n"); }
if (!-e "$param_ref->{out_dir}/intermediate_files/stat") { mkdir ("$param_ref->{out_dir}/intermediate_files/stat") || die ("Couldn't create the directory with the steps of telomeX's analysis.\nDetails: $!\n"); }
...
open (INTERMEDIATE, ">", "$param_ref->{out_dir}/intermediate_files/telomere/$param_ref->{intermediate}") || die ('Could not create all_telomere file. Please check writing permission in your current directory', "\n");
...
my $pr = String::ProgressBar->new(max => $#names);
 ...
 # Setup a callback for when a child finishes up so we can
  ...
  foreach my $child ( 0 .. $#names ) {
    ...
    findTel($names[$child], $abc{$names[$child]}{nuc_seq}, 10, 'testref', $param_ref->{mismatch}, $param_ref);
...
detectStat("$param_ref->{out_dir}/intermediate_files/telomere/$param_ref->{intermediate}", "$param_ref->{out_dir}/intermediate_files/stat/telomere.q4.stat", $param_ref->{score});
...
if ($param_ref->{overlaps} == 1) {
	print LOG "Merging all DIRECT overlaps\n";
 	blockMerger("$param_ref->{out_dir}/intermediate_files/telomere/$param_ref->{intermediate}", "$param_ref->{out_dir}/intermediate_files/telomere/final.q4");
	}
elsif ($param_ref->{overlaps} == 0) { #Keep all the info
	print LOG "You decided not to merge direct overlaps, keeping all\n";
	copy("$param_ref->{out_dir}/intermediate_files/telomere/$param_ref->{intermediate}", "$param_ref->{out_dir}/intermediate_files/telomere/final.q4");
	}
...
detectStat("$param_ref->{out_dir}/intermediate_files/telomere/final.q4", "$param_ref->{out_dir}/intermediate_files/stat/final.q4.stat", $param_ref->{score});
}
...
if ($param_ref->{check}==1) { # Set zero in conf, if not interested
	...
	if (!"$param_ref->{out_dir}/intermediate_files/telomere/final.q4") { print LOG_ERR "Missing final.q4 file; You might need to run --detect first\n";}
 	checkInGFF("$param_ref->{out_dir}/intermediate_files/telomere/final.q4", "$param_ref->{reference_genome_gff}", "$param_ref->{out_dir}/intermediate_files/telomere/final_annotated.q4", $param_ref->{extend} );

}

analyticStat("$param_ref->{out_dir}/intermediate_files/telomere/final_annotated.q4", "$param_ref->{out_dir}/intermediate_files/stat/final_annotated.q4.stat", $param_ref->{score});
}
...
#-------------------------------------------------------------------------------
print LOG "Analysis finished, closing telomeX.  \nGood bye :) \n" if $param_ref->{verbose};
